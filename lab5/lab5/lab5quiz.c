...
int main(void){
	...
	TCD0_CTRLA=0b00000001;   //16 bit number. so i have to use just the CLK
	...
	PORTD_PIN6CTRL=0b01000000;
	PORTD_PIN5CTRL=0b01000000;
	PORTD_PIN4CTRL=0b01000000;
	...
	volatile uint16_t blue = ((uint16_t)ZH << 8) | ZL;
	volatile uint16_t green = ((uint16_t)YH << 8) | YL;
	volatile uint16_t red = ((uint16_t)XH << 8) | XL;
...
void SPI(void){
	...
	PORTF_DIRSET=0b10101100; //set as output. the 1011 is SCK (SPI) enable, MOSI (SPI), and SSG (gyroscope)
